...
// contradiction is ever silently resolved (no last-writer-wins), on the REAL
// learn() write path, not just against reconcileWrite called directly.
describe("parallel-conflict & precedence (comprehensive)", () => {
  it("passes the full gated verdict with zero silent wrong winners", async () => {
  ...
  it("verifies every ordered precedence-rank pair (full cross-product)", async () => {
    const r = await runParallelConflictEval();
  ...
  it("byPrecedence is a stable total order with deterministic contentKey tiebreak", async () => {
    const r = await runParallelConflictEval();
    expect(r.determinism.stable).toBe(true);
    expect(r.determinism.tiebreakByContentKey).toBe(true);
  });

  it("resolveConflicts semantics and reconcileWrite matrix fully pass", async () => {
    const r = await runParallelConflictEval();
    expect(r.resolve.passed).toBe(r.resolve.cases);
    expect(r.reconcile.passed).toBe(r.reconcile.cases);
  });

  it("runs real Runtime concurrent writers through learn() without silent overwrites", async () => {
    const r = await runParallelConflictEval();
  ...
  it("is deterministic across runs", async () => {
    const a = await runParallelConflictEval();
    const b = await runParallelConflictEval();
